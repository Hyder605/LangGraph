...
def prompt_refinner(state: MangaState):
    ...
    genre = state.get('genre', '')  # Get the genre
    genre_instruction = f"\n- Story tone: {genre.upper()} (e.g., dramatic with high stakes/tension, or magical with wonder/fantasy elements)"
    ...
    - Keep total length ~30–70 words.
     {genre_instruction}
     -Always mention the {genre} elements in the story.What Genere is it?
...
def manga_comic_generator(state: MangaState):
    refined_story = state.get('user_edits') or state['refined_story']
    genre = state.get('genre', '')  # Get the genre
    genre_instruction = f"Genre: {genre.upper()}. " if genre else ""

    prompt = f"""
    You are a black and white Manga Illustrator AI.
     {genre_instruction} Generate compact **manga-style comic panel prompts** (≤70 tokens each).
...
def build_workflow():
    ...
    graph.add_edge("prompt_refinner", "manga_comic_generator")
    graph.add_edge("manga_comic_generator", END)
