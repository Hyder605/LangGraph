...
async def approve_story(story_edit: StoryEdit):
    ...
    try:
        ...
        # Save prompts in session
        session["prompts"] = prompts_data
        session["final_after_review"] = final_after_review

        # Create output folder
        folder_path = os.path.join("generated_images", story_edit.session_id)
        os.makedirs(folder_path, exist_ok=True)

        # Auto generate and store images for every panel
        for panel in prompts_data:
            ...
            try:
                ...
                # Save path
                file_path = os.path.join(folder_path, f"panel_{panel_num}.png")

                # Save file
                image.save(file_path, format="PNG")

            except Exception as gen_error:
                print(f"Failed to generate image for panel {panel_num}: {gen_error}")
                continue
